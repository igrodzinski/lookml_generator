...
def generate_lookml_from_excel(df,dataset_name,model_name,pre_file_name):
    ...
    for index, row in df.iterrows():
        ...
        # Check if the dimension should be commented out
        ...
        if column_name in predefined_columns:
            if "dimension_group:" in predefined_columns[column_name]:
                lookml_code_dimgr.append(comment_prefix + predefined_columns[column_name])
            elif "measure:"in predefined_columns[column_name]:
                lookml_code_m.append(comment_prefix + predefined_columns[column_name])
            else:
                lookml_code_dim.append(comment_prefix + predefined_columns[column_name])
        else:
    ...
    dimension_group: {column_name} {{
        label: "{label}"\n        {string_group_label}\n        group_item_label: "{{% assign timeframe = _field._name | remove: _view._name | remove: \'{column_name}_\' | remove: \'\' | replace: \'day_of_week\' , \'Dzień tygodnia\' | replace: \'year\' , \'Rok\' | replace: \'month\' , \'Miesiąc\' | replace: \'date\' , \'Data\' | replace: \'quarter\' , \'Kwartał\' | replace: \'week\' , \'Tydzień\' | replace: \'time\' , \'Czas\' | capitalize %}}{{{{ timeframe }}}}}"\n        description: "{description}"\n        allow_fill: yes\n        datatype: date\n        type:  time\n        timeframes: [date, day_of_week, month, quarter, year]\n        drill_fields: [{column_name}_month, {column_name}_date]\n        html: {{{{% if value==\"Monday\" %}}}}\n          Poniedziałek\n        {{{{% elsif value==\"Tuesday\" %}}}}\n          Wtorek\n        {{{{% elsif value==\"Wednesday\" %}}}}\n          Środa\n        {{{{% elsif value==\"Thursday\" %}}}}\n          Czwartek\n        {{{{% elsif value==\"Friday\" %}}}}\n          Piątek\n        {{{{% elsif value==\"Saturday\" %}}}}\n          Sobota\n        {{{{% elsif value==\"Sunday\" %}}}}\n          Niedziela\n        {{{{% else %}}}}\n          {{{{{{ rendered_value }}}}}}}\n        {{{{% endif %}}}} ;;\n        sql: ${{TABLE}}.{column_name} ;;\n    }}""".format(column_name=column_name, label=label, string_group_label=string_group_label, description=description)
                )
